  ...
  return (
    <div
      ...
      style={{
        ...
        padding: 20,
        transform: hovered ? 'translateY(-6px) scale(1.02)' : visible ? 'translateY(0) scale(1)' : 'translateY(24px) scale(0.97)',
        opacity: visible ? 1 : 0,
    ...
    >
      {hovered && (
        <div style={{
          ...
          background: `radial-gradient(circle, ${c.bg.replace('0.12', '0.2')}, transparent 70%)`,
          pointerEvents: 'none',
        }} />
      )}
      <div style={{
...
export default function FeatureCards({ items }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0,1fr))', gap: 14 }}>
      {items.map((it, i) => <AnimCard key={it.title} it={it} delay={i * 0.07} />)}
    </div>
  );
}
