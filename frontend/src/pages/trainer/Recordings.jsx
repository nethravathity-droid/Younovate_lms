// Trainer Recorded Videos — lists recordings for the trainer's own sessions
...
export default function TrainerRecordings() {
  ...
  const [fSessionType, setFSessionType] = useState(urlSessionType || 'all');
  ...
  const loadData = () => {
    ...
    setLoading(true);
    setError('');
    axios.get(`${API}/api/trainer/recordings`, { headers })
  ...
  const filtered = useMemo(() => {
    ...
    return recordings.filter(r => {
      ...
      const sessionType = r.sessionId?.sessionType || '';
      return (
        (!q || session.toLowerCase().includes(q)) &&
        (!fStatus || r.status === fStatus || (fStatus === 'available' && r.status === 'completed')) &&
        (fSessionType === 'all' || sessionType === fSessionType)
      );
    });
  }, [recordings, search, fStatus, fSessionType]);

  const isWorkshopOnly = fSessionType === 'WORKSHOP';

  return (
    <div style={S.page}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h2 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: '#0F172A' }}>{isWorkshopOnly ? 'Workshop Recordings' : 'Recorded Videos'}</h2>
          <p style={{ margin: '4px 0 0', fontSize: 13, color: '#64748B' }}>
            {recordings.length} recording{recordings.length !== 1 ? 's' : ''} from your sessions.
          </p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 10, marginBottom: 16 }}>
        <div style={{ position: 'relative' }}>
          ...
          <input style={{ ...S.input, paddingLeft: 32 }} value={search} onChange={e => setSearch(e.target.value)} placeholder="Search session…" />
        </div>
        <select style={S.input} value={fSessionType} onChange={e => setFSessionType(e.target.value)}>
        ...
        <select style={S.input} value={fStatus} onChange={e => setFStatus(e.target.value)}>
          ...
          {['none','starting','recording','processing','available','failed','aborted'].map(st => <option key={st} value={st}>{st}</option>)}</select>
      </div>
      ...
      <div style={{ ...S.card, overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 700 }}>
            <thead>
              <tr>{['Session','Type','Date','Duration','Status','Recording'].map(h => <th key={h} style={S.th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={6} style={{ ...S.td, textAlign: 'center', padding: 40, color: '#94A3B8' }}>Loading recordings…</td></tr>
              ) : filtered.length === 0 ? (
