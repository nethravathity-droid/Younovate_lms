...
// HR Settings — General, Features, Account (Change Password)
...
const CSS = `
  ...
  .hrs-wrap *, .hrs-wrap *::before, .hrs-wrap *::after { box-sizing: border-box; }
  @keyframes hrs-spin { to { transform: rotate(360deg); } }
  @keyframes hrs-fade { from { opacity:0; transform: translateY(6px); } to { opacity:1; transform: translateY(0); } }
  .hrs-input:focus, .hrs-select:focus { outline: none; border-color: ${BRAND}; box-shadow: 0 0 0 3px rgba(63,125,160,.15); }
  .hrs-tab:hover { background: #f1f6fb; color: #172033; }
  .hrs-btn:hover:not(:disabled) { opacity: .9; }
  .hrs-btn:disabled { opacity: .55; cursor: not-allowed; }

  .hrs-switch { position: relative; width: 40px; height: 22px; flex-shrink: 0; }
  .hrs-switch input { opacity: 0; width: 0; height: 0; }
  .hrs-slider { position: absolute; inset: 0; cursor: pointer; background: #cbd5e1; border-radius: 99px; transition: background .18s; }
  .hrs-slider::before { content: ''; position: absolute; height: 16px; width: 16px; left: 3px; top: 3px; background: #fff; border-radius: 50%; transition: transform .18s; }
  .hrs-switch input:checked + .hrs-slider { background: ${BRAND}; }
  .hrs-switch input:checked + .hrs-slider::before { transform: translateX(18px); }
  .hrs-switch input:disabled + .hrs-slider { opacity: .5; cursor: not-allowed; }

  @media (max-width: 820px) {
    .hrs-shell  { flex-direction: column !important; }
    .hrs-tabs   { flex-direction: row !important; overflow-x: auto; width: 100% !important; border-right: none !important; border-bottom: 1px solid #eef3f8; }
    .hrs-tab    { white-space: nowrap; border-radius: 8px !important; }
    .hrs-grid   { grid-template-columns: 1fr !important; }
    .hrs-pad    { padding: 16px !important; }
  }
`;
...
const TextField = ({ label, hint, value, onChange, type = 'text', placeholder, showToggle, onToggle, disabled }) => (
  <Field label={label} hint={hint}>
    <div style={{ position: 'relative' }}>
      <input className="hrs-input" type={type} value={value ?? ''} placeholder={placeholder}
...
const SectionShell = ({ title, desc, saving, onSave, dirty, children, saveLabel = 'Save changes' }) => (
  <div style={{ animation: 'hrs-fade .2s ease' }}>
    ...
    <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 20, borderTop: '1px solid #eef3f8', paddingTop: 16 }}>
      <button className="hrs-btn" type="button" onClick={onSave} disabled={saving || !dirty}
...
export default function HRSettings() {
    ...
    if (tab === 'general') {
      return (
        <SectionShell title="General" desc="Your profile information."
          ...
          <div className="hrs-grid" style={grid}>
      ...
      return (
        <SectionShell title="Preferences" desc="Customize your experience."
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            ...
            <ToggleField label="Interview Reminders" hint="Get reminders for scheduled interviews" checked={interviewReminders} onChange={setInterviewReminders} />
            <ToggleField label="Candidate Alerts" hint="Notifications for candidate updates" checked={candidateAlerts} onChange={setCandidateAlerts} />
          </div>
        </SectionShell>
      );
    }
  ...
  }, [tab, saving, name, email, phone, bio, emailNotifications, interviewReminders, candidateAlerts, newPassword, confirmPassword, showNewPassword, showConfirmPassword, passwordErrors, passwordSaving, generalDirty]); // eslint-disable-line react-hooks/exhaustive-deps

  return (
    <div className="hrs-wrap" style={{ padding: 28, fontFamily: 'Public Sans, system-ui, sans-serif' }}>
      <style>{CSS}</style>

      <div style={{ marginBottom: 18 }}>
        <h2 style={{ fontSize: 22, fontWeight: 900, marginBottom: 6, color: '#172033' }}>HR Settings</h2>
        <p style={{ margin: 0, color: '#657691', fontSize: 13 }}>
          Manage your profile, preferences, and account security.
        </p>
      </div>

      <div className="hrs-shell" style={{ display: 'flex', gap: 0, background: '#fff', borderRadius: 14, border: '1px solid #dbe3ed', overflow: 'hidden', boxShadow: '0 1px 2px rgba(23,32,51,.08), 0 12px 28px rgba(31,61,99,.05)' }}>
        <div className="hrs-tabs" style={{ display: 'flex', flexDirection: 'column', gap: 4, width: 210, flexShrink: 0, padding: 12, borderRight: '1px solid #eef3f8', background: '#fbfdff' }}>
          {TABS.map((t) => {
            const active = tab === t.id;
            return (
              <button key={t.id} className="hrs-tab" onClick={() => setTab(t.id)}
                style={{
        ...
        <div className="hrs-pad" style={{ flex: 1, padding: 24, minWidth: 0 }}>
          {body}
        </div>
      </div>
    </div>
